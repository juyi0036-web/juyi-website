...
import { useRouter } from 'next/router'; // 引入路由功能
import { categories } from '../data/categories';
import { translations } from '../data/translations'; // 引入翻译

export default function Navbar() {
  const [isProductOpen, setIsProductOpen] = useState(false);
  const router = useRouter();
  const { locale, locales, asPath } = router; // 获取当前语言
  
  // 获取当前语言的翻译包
  const t = translations[locale] || translations.fr;

  return (
    <nav className="bg-cream shadow-sm font-sans sticky top-0 z-50 border-b border-[#EAD8C0] relative">
        ...
        <div className="flex justify-between h-24">
          ...
          <div className="hidden md:flex space-x-8 items-center">
            ...
            >
              ...
              {/* 下拉内容 (分类暂时保持法语，因为是专业名词，通常不需要频繁翻译，或者以后再细化) */}
              {isProductOpen && (
          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            {/* 语言切换器 */}
            <div className="flex items-center space-x-2 text-sm font-bold text-choco">
              {locales.map((l) => (
                <Link 
                  key={l} 
                  href={asPath} 
            ...
            <a 
              ...
              className="bg-choco text-cream border-2 border-choco px-4 py-2 rounded-full text-sm font-bold hover:bg-cream hover:text-choco transition flex items-center no-underline shadow-sm"
            >
