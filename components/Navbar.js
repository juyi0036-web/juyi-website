        ...
        <div className="flex justify-between h-24">
          ...
          <div className="hidden md:flex flex-col items-end space-y-2">
            <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-2 text-xs font-bold text-choco">
                {locales.map((l) => (
                  <Link 
                    key={l} 
                    href={asPath} 
            ...
            <form onSubmit={onSearchSubmit} className="w-full flex justify-end">
              <input
                type="search"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder={searchPlaceholder}
                className="w-40 sm:w-48 md:w-52 px-4 py-2 bg-white border border-gray-300 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-choco focus:border-choco transition"
              />
            </form>
          </div>
        </div>
